...
ShaderManager::~ShaderManager() { FreeShaders(); }
...
IShaderProgram* ShaderManager::GetShaderProgram(
    const SimpleString& VertexShaderFilename,
    const SimpleString& PixelShaderFilename,
  ...
  const Map<SShaderProgramKey, IShaderProgram*>::Iterator ProgramIter =
      m_ShaderPrograms.Search(Key);
  if (ProgramIter.IsValid()) {
    ...
    IShaderProgram* const pShaderProgram = m_Renderer->CreateShaderProgram(
        pVertexShader, pPixelShader, pVertexDeclaration);
...
IVertexShader* ShaderManager::GetVertexShader(const SimpleString& Filename) {
  ...
  const Map<HashedString, IVertexShader*>::Iterator VSIter =
      m_VertexShaders.Search(HashedFilename);
  if (VSIter.IsValid()) {
    return VSIter.GetValue();
  } else {
    IVertexShader* const pVertexShader =
        m_Renderer->CreateVertexShader(Filename);
...
IPixelShader* ShaderManager::GetPixelShader(const SimpleString& Filename) {
  const HashedString HashedFilename = Filename;
  const Map<HashedString, IPixelShader*>::Iterator PSIter =
      m_PixelShaders.Search(HashedFilename);
  if (PSIter.IsValid()) {
