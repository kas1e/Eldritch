...
/*virtual*/ void Framework3D::GetInitialDisplaySize(uint& DisplayWidth,
                                                    uint& DisplayHeight) {
  ASSERT(m_Display);
...
/*virtual*/ void Framework3D::InitializeAudioSystem() { WARN; }

/*virtual*/ void Framework3D::InitializeRender() {}

/*virtual*/ void Framework3D::GetUIManagerDefinitionName(
...
/*virtual*/ void Framework3D::ShutDown() {
  ...
  SafeDelete(
      m_Window);  // Destroy after renderer, else I get GL errors with SDL.
  ...
  Profiler::GetInstance()->Dump(
      FileStream("profiler.txt", FileStream::EFM_Write));
...
/*virtual*/ bool Framework3D::Tick() {
  ...
  if (m_DoVideoCapture) {
    ...
    if (m_FrameTimeAccumulator > m_FrameTimeLimit) {
      ...
      // Don't bother simming for this duration, it will just cause more
      // hitches.
      m_FrameTimeAccumulator = 0.0f;
    }

    const int NumSimTicks =
        static_cast<int>(m_FrameTimeAccumulator / m_FixedFrameTime);
    const int LastSimTick = NumSimTicks - 1;
    for (int SimTickIndex = 0; SimTickIndex <= LastSimTick && DoNextTick;
         ++SimTickIndex) {
...
#if BUILD_WINDOWS_NO_SDL
  {
    XTRACE_NAMED(SwitchToThread);  // Crashing within this trace name would
                                   // typically indicate a crash in another
                                   // thread.
...
/*virtual*/ bool Framework3D::TickSim(float DeltaTime) {
  ...
  if (m_Mouse->IsActive() && m_Window->HasFocus()) {
    ...
    m_Mouse->SetPosition(m_Display->m_Width / 2, m_Display->m_Height / 2,
                         m_Window);
  }
#endif
...
/*virtual*/ void Framework3D::OnUnpaused() {
  ...
  // HACK: If we've just unpaused, tick devices again so we ignore rising inputs
  // on the next frame.
  TickDevices();
}
...
/*virtual*/ void Framework3D::DropRender() { m_Renderer->FlushBuckets(); }
...
// TODO PORT LATER: Support screenshots on other platforms.
  ...
  const SimpleString ScreenshotName =
      SimpleString::PrintF("Screens/%s.png", TimeDateString.CStr());
...
void Framework3D::RefreshWindowSize() {
  ...
  if (m_Display->m_Fullscreen) {
    ...
    // If display size is same as desktop size, and we're not fullscreen,
    // automatically switch to a borderless window.
    ...
    m_Window->SetSize(m_Display->m_Width, m_Display->m_Height,
                      m_Display->m_ScreenWidth, m_Display->m_ScreenHeight,
                      true);
...
// HACKHACK: At least for now, Linux doesn't do fullscreen at non desktop
// resolutions well.
// So switch to desktop res when going fullscreen.
  ...
  if (!NewFullScreen && (m_Display->m_Width > m_Display->m_ScreenWidth ||
  ...
  // Set size after resetting renderer to be sure screen-to-client transform is
  // correct
  RefreshWindowSize();
}

/*virtual*/ void Framework3D::SetResolution(const uint DisplayWidth,
                                            const uint DisplayHeight) {
// HACKHACK: At least for now, Linux doesn't do fullscreen at non desktop
// resolutions well.
// So if we're in fullscreen mode, promote all resolution changes to desktop
// resolution.
#if BUILD_LINUX
  if (m_Display->m_Fullscreen && (DisplayWidth != m_Display->m_ScreenWidth ||
                                  DisplayHeight != m_Display->m_ScreenHeight)) {
    SetResolution(m_Display->m_ScreenWidth, m_Display->m_ScreenHeight);
    return;
  }
#endif
  ...
  //printf("we before RefreshWindowSize()\n");
  //IDOS->Delay(100);

  RefreshWindowSize();

  //printf("we after RefreshWindowSize()\n");
  //IDOS->Delay(100);
...
void Framework3D::ConditionalRefreshDisplay() {
  ...
  ASSERT(m_Display);
  if (m_Display->NeedsUpdate() || m_Display->m_Fullscreen != Fullscreen ||
...
/*virtual*/ void Framework3D::RefreshDisplay(const bool Fullscreen,
...
bool Framework3D::HasRequestedExit() {
  ...
  while (PeekMessage(&Msg, NULL, 0, 0, PM_REMOVE)) {
    // WM_QUIT is not associated with a window and must be processed here, not
    // WndProc
  ...
  if (m_Keyboard->IsHigh(Keyboard::EB_Alt) &&
      m_Keyboard->OnRise(Keyboard::EB_F4)) {
    return true;
  }
#endif
...
#if BUILD_WINDOWS_NO_SDL
LRESULT CALLBACK
Framework3D::WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  switch (uMsg) {
    case WM_CLOSE:
    case WM_DESTROY: {
      PostQuitMessage(0);
    ...
    case WM_ACTIVATE: {
      if (wParam != WA_INACTIVE) {
        ...
        WB_DISPATCH_EVENT(gSingletonFramework->GetEventManager(),
                          ConditionalRefreshDisplay, gSingletonFramework);
      }
      break;
    }
