...
int Window::Init(const char* Title, const char* ClassName, DWORD Style,
                 DWORD ExStyle, int Width, int Height, HINSTANCE hInstance,
                 WNDPROC WndProc /*= NULL*/, uint Icon /*= 0*/,
  ...
  m_WndClassEx.hIcon =
      (Icon == 0 ? NULL : LoadIcon(hInstance, MAKEINTRESOURCE(Icon)));
  m_WndClassEx.hCursor = (HCURSOR)LoadImage(NULL, MAKEINTRESOURCE(IDC_ARROW),
  ...
  m_hWnd = CreateWindowEx(ExStyle, ClassName, Title, Style, WindowX, WindowY,
                          AdjustedWidth, AdjustedHeight, NULL, NULL,
                          m_hInstance, NULL);
  ASSERT(m_hWnd);
...
int Window::Show(int nCmdShow) {
  ...
  SetWindowPos(m_hWnd, HWND_NOTOPMOST, 0, 0, 0, 0,
               SWP_NOSIZE | SWP_NOMOVE);  // Make sure we're on top
  UpdateWindow(m_hWnd);
  return 0;
}
...
int Window::SetSize(int Width, int Height, bool AdjustForStyle) {
  ...
  // NOTE: This is just to update the size, it does *not* position window at (0,
  // 0) (SWP_NOMOVE flag).
  const BOOL Success =
...
int Window::SetSize(int Width, int Height, int ScreenWidth, int ScreenHeight,
                    bool AdjustForStyle) {
  if (!m_Inited) {
    return 0;
  }
  ...
  const BOOL Success =
      SetWindowPos(m_hWnd, HWND_NOTOPMOST, WindowX, WindowY, Width, Height, 0);
  ASSERT(Success);
  Unused(Success);
...
void Window::SetBordered(const bool Bordered) {
  ...
  const DWORD StyleFlags =
      Bordered ? (WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX)
               : WS_POPUP;
  SetStyle(StyleFlags);
#endif
...
#endif
#if BUILD_SDL
  // NOTE: SDL also has a SDL_WINDOW_FULLSCREEN_DESKTOP for borderless
