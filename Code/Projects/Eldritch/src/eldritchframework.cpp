...
/*virtual*/ void EldritchFramework::GetUIManagerDefinitionName(
    SimpleString& DefinitionName) {
...
void EldritchFramework::InitializePackages() {
  ...
  for (uint PackageIndex = 0; PackageIndex < NumPackages; ++PackageIndex) {
    const SimpleString PackageName =
        ConfigManager::GetSequenceString("Package%d", PackageIndex, "");
...
/*static*/ void EldritchFramework::OnFadeFinished(void* pUIElement,
                                                  void* pVoid) {
  Unused(pVoid);
  Unused(pUIElement);

  // HACKHACK: Handle logo fade sequence here because event manager isn't meant
  // for timed UI stuff.
...
void EldritchFramework::HandleUISliderEvent(const HashedString& SliderName,
  ...
  if (SliderName == sMouseSpeedSlider) {
    ...
    const float ControllerSpeed =
        GetControllerSpeedFromSliderValue(SliderValue);

    STATICHASH(ControllerSpeedX);
    const float ControllerSpeedX =
        ControllerSpeed * ConfigManager::GetFloat(sControllerSpeedX);

    STATICHASH(ControllerSpeedY);
    const float ControllerSpeedY =
        -ControllerSpeed *
        ConfigManager::GetFloat(sControllerSpeedY);  // HACK! Controller Y axis
                                                     // is inverted from mouse.
...
// TODO: Parameterize these functions and move to mathcore, they're pretty
// useful.
inline float EldritchFramework::GetMouseSpeedFromSliderValue(
    const float SliderValue) {
  // TODO: Parameterize this so I can tune it.
  // This should scale neatly from 1/8 to 8 on a power curve, with 1 in the
  // middle.
...
inline float EldritchFramework::GetControllerSpeedFromSliderValue(
    const float SliderValue) {
  // TODO: Parameterize this so I can tune it.
  // This should scale neatly from 1/2 to 2 on a power curve, with 1 in the
  // middle.
...
inline float EldritchFramework::GetSliderValueFromControllerSpeed(
    const float ControllerSpeed) {
  static const float sControllerScaleRange = 2.0f;
  const float AdjustedSliderValue =
      LogBase(ControllerSpeed, sControllerScaleRange);
  const float SliderValue = (AdjustedSliderValue + 1.0f) * 0.5f;
  return SliderValue;
}

inline float EldritchFramework::GetBrightnessFromSliderValue(
    const float SliderValue) {
  // TODO: Parameterize this so I can tune it.
  // This should scale neatly from 2 to 1/2 on a power curve, with 1 in the
  // middle.
...
/*virtual*/ void EldritchFramework::OnUnpaused() {
  ...
  // HACK: If we've just unpaused, tick input system again so we ignore rising
  // inputs on the next frame.
  m_InputSystem->Tick();
}
...
/*virtual*/ void EldritchFramework::TickDevices() {
  ...
  // If we're using either controller or mouse exclusively, allow or disallow
...
void EldritchFramework::Pause() {
  ...
  if (EldritchGame::IsPlayerAlive() &&
      !EldritchGame::IsPlayerDisablingPause()) {
...
/*virtual*/ bool EldritchFramework::TickInput(float DeltaTime) {
  ...
  if (m_Keyboard->OnRise(Keyboard::EB_Escape) ||
      m_Controller->OnRise(XInputController::EB_Start)) {
    Pause();
  }
...
#else
  {
    ...
    if (m_Keyboard->IsHigh(Keyboard::EB_LeftAlt) &&
        m_Keyboard->OnRise(Keyboard::EB_S)) {
      SimpleString SaveFileName;
      if (FileUtil::GetSaveFile(GetWindow()->GetHWnd(), "Save Files",
                                "eldritchmastersave", SaveFileName)) {
    ...
    // Alt + L loads
    if (m_Keyboard->IsHigh(Keyboard::EB_LeftAlt) &&
        m_Keyboard->OnRise(Keyboard::EB_L)) {
      SimpleString LoadFileName;
      if (FileUtil::GetLoadFile(GetWindow()->GetHWnd(), "Save Files",
                                "eldritchmastersave", LoadFileName)) {
        PrepareForLoad();
  ...
  // Ctrl + T is a proxy return to hub (new worlds, same hub), like Return to
  // Library.
  ...
  if (m_Keyboard->OnRise(Keyboard::EB_T) &&
    ...
    WB_SET_AUTO(ReturnToHub, Bool, FlushHub,
                m_Keyboard->IsHigh(Keyboard::EB_LeftShift));
    WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), ReturnToHub,
                      m_Game);
  } else if (m_Keyboard->OnRise(Keyboard::EB_T) &&
             m_Keyboard->IsHigh(Keyboard::EB_LeftAlt)) {
    if (m_Keyboard->IsHigh(Keyboard::EB_LeftShift)) {
      WB_MAKE_EVENT(GoToPrevLevel, NULL);
      WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(),
                        GoToPrevLevel, m_Game);
    } else {
      WB_MAKE_EVENT(GoToNextLevel, NULL);
      WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(),
                        GoToNextLevel, m_Game);
    }
  } else if (m_Keyboard->OnRise(Keyboard::EB_T)) {
    RegenerateWorld();
...
/*virtual*/ void EldritchFramework::TickPausedInput() {
  ...
  if (m_InputSystem->IsBinding()) {
    ...
    if (!m_InputSystem->IsBinding()) {
      ...
      if (m_UIManager->GetUIStack()->Top() ==
          m_UIManager->GetScreen(sBindDialog)) {
...
void EldritchFramework::SetMainViewTransform(const Vector& Location,
  ...
  new (m_MainView) View(Vector(25.0f, 25.0f, 13.0f), Vector(0.0f, 1.0f, 0.0f),
                        SRect(-25.0f, 13.0f, 25.0f, -13.0f), 0.0f, 50.0f);
#endif
}
...
/*virtual*/ void EldritchFramework::SetResolution(const uint DisplayWidth,
  ...
  //printf("we before SetResolution\n");
  //IDOS->Delay(100);

  Framework3D::SetResolution(DisplayWidth, DisplayHeight);

  //printf("we after SetResolution, and before CreateTargets\n");
  //IDOS->Delay(100);

  m_TargetManager->CreateTargets(DisplayWidth, DisplayHeight);

  //printf("we after CreateTargets and before UpdateViews\n");
  //IDOS->Delay(100);
...
#undef BUCKET
  ...
  // Get buckets by name and set properties here because I didn't want to change
  // that horrid bucket constructor
...
void EldritchFramework::CreateHUDView() {
  ...
  m_HUDView = new View(EyePosition, EyeDirection,
                       SRect(0.0f, 0.0f, fDisplayWidth, fDisplayHeight),
                       NearClip, FarClip);
}
...
void EldritchFramework::CreateMirrorView() {
  ...
  const float fMirrorRTWidth =
      static_cast<float>(pMirrorScreen->GetMirrorRTWidth());
  const float fMirrorRTHeight =
      static_cast<float>(pMirrorScreen->GetMirrorRTHeight());
  const float AspectRatio = fMirrorRTWidth / fMirrorRTHeight;
  const Matrix MirrorRotation = Matrix::CreateRotationAboutZ(
      pMirrorScreen->GetMirrorYaw());  // For beauty lighting with cube lights
  const Vector EyePosition =
      MirrorRotation * Vector(0.0f, pMirrorScreen->GetMirrorViewDistance(),
                              pMirrorScreen->GetMirrorViewHeight());
  ...
  m_MirrorView =
      new View(EyePosition, EyeDirection, FOV, AspectRatio, NearClip, FarClip);
}
...
/*virtual*/ void EldritchFramework::RefreshDisplay(const bool Fullscreen,
